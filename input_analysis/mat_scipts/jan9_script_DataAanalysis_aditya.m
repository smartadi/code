...
close all
j=50
[a i] = min(abs(t - stimStarts_a(j)));
[a i2] = min(abs(t - stimEnds_a(j)));
figure()
...
close all
j=50
i = input_params(j,2) - horizon;
i2 = input_params(j,2) - horizon + dur*35;
figure()
plot(t(i-35:i+35*4),dFk(i-35:i+35*4));hold on
plot(t(i-35:i+35*4),dFk(i-35:i+35*4))
xline(t(i))
xline(t(i2))
...
ncDfk=[];
for j = 1: length(nc)
    [a i] = min(abs(t - stimStarts_a(nc(j))));
...
wcDfk=[];
for j = 1: length(wc)
    [a i] = min(abs(t - stimStarts_a(wc(j))));
...
hgexport(gcf,'images/comp_controllers01091.pdf',figure_property); %Set desired file name
...
figure_property.Width= '16'; % Figure width on canvas
figure_property.Height= '9'; % Figure height on canvas
hgexport(gcf,'images/comp_error12191.pdf',figure_property); %Set desired file name
%%
% %%%%%%%%%%%%%%%%%%%%%%
close all;
figure(1)
...
figure_property.Width= '16'; % Figure width on canvas
figure_property.Height= '9'; % Figure height on canvas
hgexport(gcf,'images/comp_var_error12191.pdf',figure_property); %Set desired file name
...
blue_exp = readNPY(append(serverRoot,'/blueLEDmonitor.raw.npy'));
violet_exp = readNPY(append(serverRoot,'/violetLEDmonitor.raw.npy'));
%%
close all
figure()
plot(tt,blue_exp)
