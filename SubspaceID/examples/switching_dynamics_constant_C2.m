...
cvx_begin
    ...
    for i=1:T
        ...
        % size(C)
        obj = sum(norm([Th((i-1)*(nn)+1:(i)*(nn),:);C]*RHS((i-1)*nn+1:i*nn,:)-LHS((i-1)*(nn+l)+1:i*(nn+l),:)) + gamma*norm(Th((i-1)*(nn)+1:(i)*(nn),:),1));
    end
...
%             norm(Th((i-1)*(nn+l)+nn+1:(i-1)*(nn+l)+nn+l,:) - Th((i)*(nn+l)+nn+1:(i)*(nn+l)+nn+l,:)) <= alpha;
%         end
% 
% 
% 
