...
function [A,B,C,D,K,Ro,AUX,ss] = subid_sparse2(y,u,i,n,AUXin,W,sil)
...
% Solve least square with sparsity:
cvx_begin
    %lambda = 1e-3;
    lambda = 1;
    variable Sol(n+l,n)
    minimize( norm(Sol*Rhs-Lhs,2) + lambda*norm([eye(n,n),zeros(n,l)]*Sol,1) )
    %minimize( norm(Sol*Rhs-Lhs) + lambda*norm(Sol,1) )
...
A = Sol(1:n,1:n);
C = Sol(n+1:n+l,1:n);
%A = gam_inv*[mgam;zeros(l,size(mgam,2))];
%C = gam(1:l,:);
sol = [A;C];
res = Lhs - sol*Rhs; 			% Residuals
